...
jQuery(document).ready(function ($) {
	...
	$(document).ajaxStop(function () {
		...
		// START Changelog buttons
		$('.changelog_button').click(function () {
			$('html, body').animate({ // 70 is the navbar height.
				scrollTop : $('#changelog').offset().top - 70
			}, 'slow');
			return false;
		});
		// END Changelog buttons
...
function spoiler(aNode) {
	if (aNode.value === "Show more") {
		$(aNode.parentNode.parentNode.getElementsByTagName("span")[1].getElementsByTagName("span")[0]).fadeToggle(250, function () {
			$(this).css({
				"display" : "block"
			});
		});
		...
		$(aNode.parentNode.parentNode.getElementsByTagName("span")[1].getElementsByTagName("span")[0]).fadeToggle(250);
